...
describe('ejs.render(str, options)', function() {
  ...
  it('should accept locals', function() {
    ejs.render('<p><%= name %></p>', {name: 'tobi'}).should.equal('<p>tobi</p>');
  });
});
...
describe('<%=', function() {
  it('should escape &amp;<script>', function() {
    ejs.render('<%= name %>', {name: '&nbsp;<script>'}).should.equal('&amp;nbsp;&lt;script&gt;');
  });
  it("should escape '", function() {
    ejs.render('<%= name %>', {name: "The Jones's"}).should.equal('The Jones&#39;s');
  });
  it("should escape &foo_bar;", function() {
    ejs.render('<%= name %>', {name: "&foo_bar;"}).should.equal('&amp;foo_bar;');
  });
});
describe('<%-', function() {
...
describe('%>', function() {
  it('should produce newlines', function() {
    ejs.render(fixture('newlines.ejs'), {users: users}).should.equal(fixture('newlines.html'));
  });
});
describe('-%>', function() {
  it('should not produce newlines', function() {
    ejs.render(fixture('no.newlines.ejs'), {users: users}).should.equal(fixture('no.newlines.html'));
  });
});
...
describe('messed up whitespace', function() {
  it('should work', function() {
    ejs.render(fixture('messed.ejs'), {users: users}).should.equal(fixture('messed.html'));
  });
});
describe('filters', function() {
  it('should work', function() {
    var items = ['foo', 'bar', 'baz'];
    ejs.render('<%=: items | reverse | first | reverse | capitalize %>', {items: items}).should.equal('Zab');
  });
  it('should accept arguments', function() {
    ejs.render('<%=: users | map:"name" | join:", " %>', {users: users}).should.equal('tobi, loki, jane');
  });
  it('should truncate string', function() {
    ejs.render('<%=: word | truncate: 3 %>', {word: 'World'}).should.equal('Wor');
  });
  it('should append string if string is longer', function() {
    ejs.render('<%=: word | truncate: 2,"..." %>', {word: 'Testing'}).should.equal('Te...');
  });
  it('should not append string if string is shorter', function() {
    ejs.render('<%=: word | truncate: 10,"..." %>', {word: 'Testing'}).should.equal('Testing');
  });
